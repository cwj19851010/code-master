...
public class TemplateModule : ISeedModule
{
    ...
    public async Task AddInitialDataAsync(CodeMasterDbContext dbContext)
    {
        ...
        if (hasData)
        {
            var addedControls = await AddMissingControlTemplatesAsync(dbContext);
            Console.WriteLine(addedControls == 0
                ? "  - 页面模板数据已存在，跳过"
                : $"  ✓ 补充 {addedControls} 个缺失的控件模板");
            return;
        }
        ...
        string empty, string dict, string selectTable, string editor, string file, string image)
    {
        return new[]
        {
            ...
            ("checkbox", "add",    "<el-col [field.col]><el-form-item :label=\"$t('[field.labelKey]')\" [field.prop] data-gen-id=\"gen_field_[field.id]\"><el-checkbox v-model=\"[field.formPrefix].[field.nameLower]\">{{ $t('[field.labelKey]') }}</el-checkbox></el-form-item></el-col>", empty, 9),
            ("checkbox", "edit",   "<el-col [field.col]><el-form-item :label=\"$t('[field.labelKey]')\" [field.prop] data-gen-id=\"gen_field_[field.id]\"><el-checkbox v-model=\"[field.formPrefix].[field.nameLower]\">{{ $t('[field.labelKey]') }}</el-checkbox></el-form-item></el-col>", empty, 10),
            ("radio-group", "add",  "<el-col [field.col]><el-form-item :label=\"$t('[field.labelKey]')\" [field.prop] data-gen-id=\"gen_field_[field.id]\"><el-radio-group v-model=\"[field.formPrefix].[field.nameLower]\"><el-radio v-for=\"d in [field.nameLower]Options\" :key=\"d.value\" :value=\"d.value\">{{ d.label }}</el-radio></el-radio-group></el-form-item></el-col>", dict, 11),
            ("radio-group", "edit", "<el-col [field.col]><el-form-item :label=\"$t('[field.labelKey]')\" [field.prop] data-gen-id=\"gen_field_[field.id]\"><el-radio-group v-model=\"[field.formPrefix].[field.nameLower]\"><el-radio v-for=\"d in [field.nameLower]Options\" :key=\"d.value\" :value=\"d.value\">{{ d.label }}</el-radio></el-radio-group></el-form-item></el-col>", dict, 12),
            ("checkbox-group", "add",  "<el-col [field.col]><el-form-item :label=\"$t('[field.labelKey]')\" [field.prop] data-gen-id=\"gen_field_[field.id]\"><el-checkbox-group v-model=\"[field.formPrefix].[field.nameLower]List\"><el-checkbox v-for=\"d in [field.nameLower]Options\" :key=\"d.value\" :value=\"d.value\">{{ d.label }}</el-checkbox></el-checkbox-group></el-form-item></el-col>", dict, 13),
            ("checkbox-group", "edit", "<el-col [field.col]><el-form-item :label=\"$t('[field.labelKey]')\" [field.prop] data-gen-id=\"gen_field_[field.id]\"><el-checkbox-group v-model=\"[field.formPrefix].[field.nameLower]List\"><el-checkbox v-for=\"d in [field.nameLower]Options\" :key=\"d.value\" :value=\"d.value\">{{ d.label }}</el-checkbox></el-checkbox-group></el-form-item></el-col>", dict, 14),
            ...
            ("image",        "list",   "<el-table-column [field.prop] :label=\"$t('[field.labelKey]')\" min-width=\"180\" data-gen-id=\"gen_col_[field.id]\"><template #default=\"scope\"><div v-if=\"getUploadValues([field.rowPrefix].[field.nameLower]).length\" class=\"cm-file-link-list\"><el-link v-for=\"(u,i) in getUploadValues([field.rowPrefix].[field.nameLower])\" :key=\"i\" :href=\"u\" :download=\"getUploadFileName(u, i)\" target=\"_blank\" type=\"primary\" class=\"mr-1\">{{ getUploadFileName(u, i) }}</el-link></div><span v-else>-</span></template></el-table-column>", empty, 20),
            ...
            ("editor",       "list",   "<el-table-column [field.prop] :label=\"$t('[field.labelKey]')\" min-width=\"240\" data-gen-id=\"gen_col_[field.id]\"><template #default=\"scope\"><div class=\"rich-text-cell\" style=\"max-height:72px;overflow:hidden;line-height:1.5\" v-html=\"[field.rowPrefix].[field.nameLower] || '-'\"></div></template></el-table-column>", empty, 20),
            ("input",        "detail", "<el-descriptions-item :label=\"$t('[field.labelKey]')\" data-gen-id=\"gen_field_[field.id]\">{{ [field.detailPrefix].[field.nameLower] }}</el-descriptions-item>", empty, 21),
            ("editor",       "detail", "<el-descriptions-item :label=\"$t('[field.labelKey]')\" :span=\"2\" data-gen-id=\"gen_field_[field.id]\"><div class=\"rich-text-detail\" style=\"max-width:100%;overflow:auto;line-height:1.6\" v-html=\"[field.detailPrefix].[field.nameLower] || '-'\"></div></el-descriptions-item>", empty, 21),
            // advanced controls (add/edit only)
            ("select-table", "add",  "<el-col [field.col]><el-form-item :label=\"$t('[field.labelKey]')\" [field.prop] data-gen-id=\"gen_field_[field.id]\"><el-select v-model=\"[field.formPrefix].[field.nameLower][field.multipleSuffix]\" style=\"width:100%\" clearable [field.multipleAttr] :placeholder=\"$t('[field.labelKey]')\"><el-option v-for=\"item in [field.relatedEntityNameLower]Options\" :key=\"item.value\" :label=\"[field.relatedDisplayLabelExpr]\" :value=\"item.value\"><span [v-for=\"displayField in field.displayFields\"] style=\"margin-right:8px\">{{ [displayField.itemValueExpr] }}</span></el-option></el-select></el-form-item></el-col>", selectTable, 21),
            ("select-table", "edit", "<el-col [field.col]><el-form-item :label=\"$t('[field.labelKey]')\" [field.prop] data-gen-id=\"gen_field_[field.id]\"><el-select v-model=\"[field.formPrefix].[field.nameLower][field.multipleSuffix]\" style=\"width:100%\" clearable [field.multipleAttr] :placeholder=\"$t('[field.labelKey]')\"><el-option v-for=\"item in [field.relatedEntityNameLower]Options\" :key=\"item.value\" :label=\"[field.relatedDisplayLabelExpr]\" :value=\"item.value\"><span [v-for=\"displayField in field.displayFields\"] style=\"margin-right:8px\">{{ [displayField.itemValueExpr] }}</span></el-option></el-select></el-form-item></el-col>", selectTable, 22),
            ...
            // detail — 补全
            ("select",       "detail", "<el-descriptions-item :label=\"$t('[field.labelKey]')\" data-gen-id=\"gen_field_[field.id]\">{{ getDictLabel([field.detailPrefix].[field.nameLower], [field.nameLower]Options) }}</el-descriptions-item>", dict, 35),
            ...
            ("image",        "detail", "<el-descriptions-item :label=\"$t('[field.labelKey]')\" data-gen-id=\"gen_field_[field.id]\"><div v-if=\"getUploadValues([field.detailPrefix].[field.nameLower]).length\" class=\"cm-file-link-list\"><el-link v-for=\"(u,i) in getUploadValues([field.detailPrefix].[field.nameLower])\" :key=\"i\" :href=\"u\" :download=\"getUploadFileName(u, i)\" target=\"_blank\" type=\"primary\" class=\"mr-1\">{{ getUploadFileName(u, i) }}</el-link></div><span v-else>-</span></el-descriptions-item>", empty, 36),
            ("file",         "detail", "<el-descriptions-item :label=\"$t('[field.labelKey]')\" data-gen-id=\"gen_field_[field.id]\"><div v-if=\"getUploadValues([field.detailPrefix].[field.nameLower]).length\" class=\"cm-file-link-list\"><el-link v-for=\"(u,i) in getUploadValues([field.detailPrefix].[field.nameLower])\" :key=\"i\" :href=\"u\" :download=\"getUploadFileName(u, i)\" target=\"_blank\" type=\"primary\" class=\"mr-1\">{{ getUploadFileName(u, i) }}</el-link></div><span v-else>-</span></el-descriptions-item>", empty, 37),
            ("cascader",     "detail", "<el-descriptions-item :label=\"$t('[field.labelKey]')\" data-gen-id=\"gen_field_[field.id]\">{{ [field.detailPrefix].[field.nameLower] }}</el-descriptions-item>", empty, 38),
            ("date",         "detail", "<el-descriptions-item :label=\"$t('[field.labelKey]')\" data-gen-id=\"gen_field_[field.id]\">{{ formatDate([field.detailPrefix].[field.nameLower]) }}</el-descriptions-item>", empty, 39),
            ("datetime",     "detail", "<el-descriptions-item :label=\"$t('[field.labelKey]')\" data-gen-id=\"gen_field_[field.id]\">{{ formatDate([field.detailPrefix].[field.nameLower], true) }}</el-descriptions-item>", empty, 40),
            ("switch",       "detail", "<el-descriptions-item :label=\"$t('[field.labelKey]')\" data-gen-id=\"gen_field_[field.id]\"><el-tag :type=\"[field.detailPrefix].[field.nameLower] ? 'success' : 'danger'\">{{ [field.detailPrefix].[field.nameLower] ? $t('yes') : $t('no') }}</el-tag></el-descriptions-item>", empty, 41),
    ...
    private static async Task<int> AddMissingControlTemplatesAsync(CodeMasterDbContext dbContext)
    {
        var emptyScript = JsonSerializer.Serialize(new { imports = Array.Empty<object>(), uses = Array.Empty<object>(), refs = Array.Empty<object>(), reactives = Array.Empty<object>(), functions = Array.Empty<object>(), hooks = Array.Empty<object>(), computed = Array.Empty<object>(), watches = Array.Empty<object>() });
        var controls = GetControlTemplates(
            emptyScript,
            GetSelectDictScript(),
            GetSelectTableScript(),
            GetEditorScript(),
            GetFileScript(),
            GetImageScript());
        var existing = await dbContext.Set<SysFieldControlTemplate>()
            .Select(item => new { item.ControlType, item.PageSection })
            .ToListAsync();
        var keys = existing
            .Select(item => $"{item.ControlType}:{item.PageSection}")
        ...
        {
            if (!keys.Add($"{control.Type}:{control.Section}")) continue;

            await dbContext.Set<SysFieldControlTemplate>().AddAsync(new SysFieldControlTemplate
            {
                Id = YitIdHelper.NextId(),
                ControlType = control.Type,
                PageSection = control.Section,
                HtmlContent = control.Html,
                ScriptSections = control.Script,
                Sort = control.Sort,
                CreateTime = now
            });
            added++;
