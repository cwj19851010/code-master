    ...
    where TGetListInput : PagedQueryDto
{
    ...
    protected ReadOnlyTreeApplicationService(
    ...
    {
        var entities = await (await CreateFilteredQueryAsync(input)).ToListAsync();
        return BuildTree(entities.Adapt<List<TGetListOutputDto>>());
    }
    ...
    {
        var entity = await Repository.GetByIdAsync(id);
        if (entity == null || string.IsNullOrWhiteSpace(entity.Ancestors))
            return new List<TGetListOutputDto>();

        var ids = entity.Ancestors.Split(',', StringSplitOptions.RemoveEmptyEntries)
            .Where(value => value != "0")
            .Select(long.Parse)
            .ToList();
        if (ids.Count == 0) return new List<TGetListOutputDto>();

        return (await _db.Queryable<TEntity>()
                .Where(item => ids.Contains(item.Id))
                .OrderBy(item => item.Id)
                .ToListAsync())
            .Adapt<List<TGetListOutputDto>>();
    }
