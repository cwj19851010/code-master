      ...
      ISysLoginLogService
{
    ...
    public SysLoginLogService(IRepository<SysLoginLog> repository, IExcelService excelService, CodeMaster.Core.Services.ICacheService? cacheService = null)
    ...
    protected override Task<ISugarQueryable<SysLoginLog>> CreateFilteredQueryAsync(SysLoginLogQueryDto input)
    {
        ...
        queryable = queryable
            ...
            .WhereIF(input.BeginTime.HasValue, log => log.LoginTime >= input.BeginTime!.Value)
            .WhereIF(input.EndTime.HasValue, log => log.LoginTime <= input.EndTime!.Value);
