      ...
      ISysOperLogService
{
    ...
    public SysOperLogService(IRepository<SysOperLog> repository, IExcelService excelService, CodeMaster.Core.Services.ICacheService? cacheService = null)
    ...
    protected override Task<ISugarQueryable<SysOperLog>> CreateFilteredQueryAsync(SysOperLogQueryDto input)
    {
        ...
        queryable = queryable
            ...
            .WhereIF(input.BusinessType.HasValue, log => log.BusinessType == input.BusinessType!.Value)
            .WhereIF(input.Status.HasValue, log => log.Status == input.Status!.Value)
            .WhereIF(input.BeginTime.HasValue, log => log.OperTime >= input.BeginTime!.Value)
            .WhereIF(input.EndTime.HasValue, log => log.OperTime <= input.EndTime!.Value);
