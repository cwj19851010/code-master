      ...
      ISysTaskLogService
{
    ...
    public SysTaskLogService(IRepository<SysTaskLog> repository, IExcelService excelService, CodeMaster.Core.Services.ICacheService? cacheService = null)
    ...
    protected override Task<ISugarQueryable<SysTaskLog>> CreateFilteredQueryAsync(SysTaskLogQueryDto input)
    {
        ...
        queryable = queryable
            .WhereIF(input.TaskId.HasValue, log => log.TaskId == input.TaskId!.Value)
            ...
            .WhereIF(input.BeginTime.HasValue, log => log.CreateTime >= input.BeginTime!.Value)
            .WhereIF(input.EndTime.HasValue, log => log.CreateTime <= input.EndTime!.Value);
