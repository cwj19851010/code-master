...
public class SysDictDataService : ISysDictDataService
{
    ...
    {
        var queryable = _db.Queryable<SysDictData>()
            .WhereIF(!string.IsNullOrWhiteSpace(query.DictType), d => d.DictType.Contains(query.DictType!))
            .WhereIF(!string.IsNullOrWhiteSpace(query.Label), d => d.Label.Contains(query.Label!))
        ...
        {
            Items = items.Adapt<List<SysDictDataDto>>(),
            Total = total.Value,
            PageNum = query.PageNum,
            PageSize = query.PageSize
        };
