...
public class SysUserService : ISysUserService
{
    ...
    public async Task<PagedResultDto<SysUserDto>> GetPagedListAsync(SysUserQueryDto query)
    {
        var queryable = _db.Queryable<SysUser>()
            .WhereIF(!string.IsNullOrWhiteSpace(query.UserName), u => u.UserName.Contains(query.UserName!))
            .WhereIF(!string.IsNullOrWhiteSpace(query.PhoneNumber), u => u.PhoneNumber != null && u.PhoneNumber.Contains(query.PhoneNumber!))
            .WhereIF(query.Status.HasValue, u => u.Status == query.Status!.Value)
            .WhereIF(query.DeptId.HasValue, u => u.DeptId == query.DeptId!.Value)
            .WhereIF(query.BeginTime.HasValue, u => u.CreateTime >= query.BeginTime!.Value)
