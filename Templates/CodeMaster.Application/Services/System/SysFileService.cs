...
public class SysFileService : ISysFileService
{
    ...
    public async Task<SysFileDto> SaveFileToLocalAsync(IFormFile file, string? category = null)
    {
        ...
        // 获取配置
        var maxSize = _configuration.GetValue<long>("Upload:MaxSize", 10485760); // 默认10MB

        // 检查文件大小
        if (file.Length > maxSize)
        {
            throw new Exception($"文件大小不能超过 {maxSize / 1024 / 1024}MB");
        }

        // 获取文件扩展名
        var fileExt = Path.GetExtension(file.FileName).ToLower();
        var allowedExtensions = _configuration.GetSection("Upload:AllowedExtensions").Get<string[]>()
            ?? new[] { ".jpg", ".png", ".pdf", ".doc", ".docx", ".xls", ".xlsx" };
        ...
        var serverUrl = _configuration["Upload:ServerUrl"] ?? _configuration["Upload:UploadUrl"];
