...
public class SysRoleService : ISysRoleService
{
    ...
    public async Task<PagedResultDto<SysRoleDto>> GetPagedListAsync(SysRoleQueryDto query)
    {
        var result = await _roleRepository.GetPagedListAsync(
            where: r =>
                (string.IsNullOrEmpty(query.RoleName) || r.RoleName.Contains(query.RoleName)) &&
                (string.IsNullOrEmpty(query.RoleKey) || r.RoleKey.Contains(query.RoleKey)) &&
                (query.Status == null || r.Status == query.Status.Value) &&
                (query.BeginTime == null || r.CreateTime >= query.BeginTime.Value) &&
                (query.EndTime == null || r.CreateTime <= query.EndTime.Value),
