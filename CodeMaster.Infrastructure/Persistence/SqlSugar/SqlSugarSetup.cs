...
{
    private static readonly ConcurrentDictionary<int, bool> _registeredInstances = new();
    /// <summary>
    /// 配置SqlSugar
    /// </summary>
        ...
        ILogger? logger = null)
    {
        ...
        // 获取实例的唯一标识
        var instanceId = db.GetHashCode();

        // 只为每个实例注册一次 AOP 事件
        if (_registeredInstances.TryAdd(instanceId, true))
        {
            // 配置SQL日志
    ...
    /// <summary>
    /// 获取全局配置的外部服务（表名/列名映射），供目标项目数据库连接复用
    /// </summary>
    public static ConfigureExternalServices GetConfigureExternalServices(DbType dbType)
    {
    ...
    /// 获取数据库类型映射
    /// </summary>
    public static string GetDatabaseType(Type type, DbType dbType)
    {
        // 处理可空类型
